...
/**
 * Performance test for the ProductAccount entity.
 */
class ProductAccountGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProductAccount entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all productAccounts")
            .get("/api/product-accounts")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
